            ...
            if ($stmt->execute()) {
                ...
                if ($result->num_rows == 1) {
                    ...
                    if (password_verify($password, $hashed_password)) {
                        ...
                        if ($stmt_favs = $db->prepare($sql_favs)) {
                            $stmt_favs->bind_param("i", $user_id);
                            $stmt_favs->execute();
                            $result_favs = $stmt_favs->get_result();
                            while ($row_fav = $result_favs->fetch_assoc()) {
                                $_SESSION['favorites'][] = $row_fav['PetID'];
                            }
                            $stmt_favs->close();
                        }

                        if ($row['username'] === 'BPRV_admin') {
                            header("location: ../dashboard.php");
                        } else {
...
<body>
    <div class="wrapper">
        <a href="../index.php" style="text-decoration: none; color: inherit; display: block; text-align: center;">
        <img src="../images/BPRV_Logo.png" alt="BPRV Adoption Center Logo" style="width: 100px;">
        <p style="margin-bottom: 10px; font-size: 10px;">Click to return home</p>
        </a>
        <h2>Welcome Back!</h2>
